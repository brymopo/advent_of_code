...
Part one: Each line gives the password policy and then the password. The password policy indicates the lowest and 
...
Part two: Each policy actually describes two positions in the password, where 1 means the first character, 2 means 
the second character, and so on. (Be careful; Toboggan Corporate Policies have no concept of "index zero"!) Exactly 
one of these positions must contain the given letter. Other occurrences of the letter are irrelevant for the purposes 
...
# Getting data for puzzle

puzzle_input = '''4-6 b: bbbdbtbbbj
1-6 g: ggvggbgggstg
1-4 s: lssss
...
def get_password_and_policy(entry):
    # Input has this structure: '3-4 j: tjjj'
    # Output will look like this: ('j',[3,4],'tjjj') 
       
    password_and_policy = entry.split(':')
    policy = password_and_policy[0].split(' ')    
    policy_letter = policy[1]
    policy_bounds = [int(number) for number in policy[0].split('-')]
    password = password_and_policy[1].strip(' ')

    return (policy_letter, policy_bounds, password)

def check_password(letter, bounds, password, part_one=True):
    lower_bound = bounds[0]
    upper_bound = bounds[1]
    count = [chr for chr in password if chr == letter]

    if part_one:
        #Logic to handle the part one of the problem
        if len(count) in range(lower_bound, upper_bound + 1): 
            return True
        return False

    #Logic to handle part two of the problem
    lower_bound -= 1
    upper_bound -= 1
    
    try:
        return (password[lower_bound] == letter or password[upper_bound] == letter) and password[lower_bound] != password[upper_bound]
    except IndexError:
        return False

#Solution

formatted_input = [get_password_and_policy(entry) for entry in puzzle_input.split('\n')]
