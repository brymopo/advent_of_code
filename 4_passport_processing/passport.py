...
puzzle_input = """ecl:#eef340 eyr:2023 hcl:#c0946f pid:244684338 iyr:2020 cid:57 byr:1969 hgt:152cm
...
def validate_passport(passport): 
    # Part one Solution   
    passport_dict = {key:value for key,value in [ [data for data in pair.split(':')] for pair in passport.split(' ')]}
    mandatory_keys = ['byr','iyr','eyr','hgt','hcl','ecl','pid']
    for key in mandatory_keys:
        valid = passport_dict.get(key)
        if not valid: return False
    return True

def validate_range(number,lower,higher):    
    return lower <= number <= higher

def validate_year(year,lower,higher):
    if len(year) != 4: return False
    try:        
        return validate_range(int(year),lower,higher)
    except ValueError:
        return False
...
def validate_hair_color(hcl):
    ...
    count = [ltr for ltr in hcl[1::].lower() if (validate_range(ord(ltr),48,57) or validate_range(ord(ltr),97,102))]
    return len(count) == 6 

def validate_height(hgt):
    measure_unit = hgt[-2::].lower()    
    if measure_unit != 'cm' and measure_unit != 'in': return False
    try:
        value = int(hgt[0:-2])
        if measure_unit == 'cm': return validate_range(value,150,193)
        if measure_unit == 'in': return validate_range(value,59,76)        
    except ValueError:
        return False
    return True

def validate_passport_strict(passport):
    passport_dict = {key:value for key,value in ( ( data for data in key_value_pair.split(':')) for key_value_pair in passport.split(' '))}
    try:
        return all([
            validate_year(passport_dict.get('byr'),1920,2002),        
            validate_year(passport_dict.get('iyr'),2010,2020),        
            validate_year(passport_dict.get('eyr'),2020,2030),        
            validate_height(passport_dict.get('hgt')),
            validate_hair_color(passport_dict.get('hcl')),
            validate_eye_color(passport_dict.get('ecl')),
            validate_pid(passport_dict.get('pid'))
        ])
    except (ValueError,TypeError):        
        return False

def validate_all_passports():
    valid_passports = 0
